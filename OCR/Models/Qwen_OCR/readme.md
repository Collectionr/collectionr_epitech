...
Impact : `name` 13/15 → 14/15, identification 60 % → 66,7 %, CER 0,096 → 0,093.
...
> une convention d'écriture, pas une erreur de lecture.
>
...
|---|---|---|---|
| Exact Match (champs critiques) | ≥ 0,80 | 0,667 | ❌ |
| Field Coverage | ≥ 0,90 | 1,000 | ✅ |
| JSON Validity Rate | ≥ 0,95 | 1,000 | ✅ |
| Hallucination Rate | ≤ 0,05 | 1,000 | ❌ |
| Latence médiane | ≤ 3 000 ms | 4 397 ms | ❌ |
| CER global | ≤ 0,08 | 0,093 | ❌ |

---

## Conclusion

**Fine-tuning nécessaire : OUI.** Quatre seuils sur six ne sont pas franchis.
...
Cette conclusion mérite toutefois une nuance importante. Sur les quatre seuils en
échec, deux ont une cause identifiée qui n'est pas le modèle lui-même :
...
|---|---|---|---|
| 1 | Ajouter les formats `TG` aux exemples du prompt | Exact Match, CER | quelques minutes |
| 2 | Rejouer le benchmark en quantization AWQ ou GPTQ | Latence | une demi-journée |
| 3 | Ajouter une validation en aval : contrôle du format `XXX/YYY` et recoupement TCGdex avant d'accepter un champ | Hallucination | un jour |
| 4 | Étendre le jeu de test à 50 cartes annotées comme l'exige §8.1 | fiabilité de la mesure | plusieurs jours |

L'action 3 mérite une précision : elle ne fera pas baisser le taux
d'hallucination du modèle, qui restera le même. Elle empêche l'hallucination
d'atteindre la base de données — ce qui est l'objectif réel.
...
7. **3 champs sur 7** — voir le périmètre ci-dessus.
