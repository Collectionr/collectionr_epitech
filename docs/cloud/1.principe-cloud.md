...
Le projet Collectionr, développé par une équipe de 9 étudiants, propose une application fiable dédiée aux collectionneurs de cartes (TCG). Ce document détaille une architecture conçue pour être simple, économique et efficace, s'appuyant sur des standards professionnels pour répondre aux exigences techniques du projet.

Cette approche repose sur une conteneurisation intelligente évoluant de Docker vers k3s, une protection rigoureuse des données via le principe de Privacy by Design, ainsi qu’une automatisation complète des cycles de développement grâce à une CI/CD intégrée et une gestion systématique des Pull Requests.
...
- Le système permet de détecter rapidement les erreurs et les pannes
...
- **Base de données** : conçue pour gérer un volume de données croissant
...
- Si un service tombe, les données déjà enregistrées ne sont pas impactées.
...
- **Automatisation de l’infrastructure** : dans un premier temps, la configuration restera simple. Des outils comme Terraform pourront être étudiés plus tard si le besoin apparaît. 
...
Pour garantir la robustesse du système tout en maîtrisant les coûts, les bonnes pratiques Cloud suivantes sont sont définies et suivies par l’équipe :
...
- **Optimisation des coûts (FinOps)** : Suivi de la consommation des ressources pour éviter les dépenses inutiles.
- **Observabilité** : Mise en place de mécanismes pour observer les logs et détecter les problèmes
