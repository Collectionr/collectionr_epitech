...
L'accès aux logs est adapté selon l'environnement de déploiement, en cohérence avec notre stratégie d'environnements (D01-environnement.md) :
...
- **Resource Limits & Requests (CPU/RAM) :**
  ...
  - *Exemple type pour le Worker OCR Python en production :*
    ```yaml
    resources:
      requests:
        memory: "512Mi"
